...
export default ({
  ...
  function creatAppConfigFile(filePath: string) {
    const _CustomEnv: VersionEnvSpace.EnvConfig = JSON.parse(JSON.stringify(CustomEnv));
  ...
  return {
    ...
    transformIndexHtml: {
      handler(html) {
        ...
        // title
        html = html.replace(/<title>(.*?)<\/title>/, `<title>${CustomEnv.GLOBAL_CONFIG.web_tilte || ''}</title>`);
        return {
